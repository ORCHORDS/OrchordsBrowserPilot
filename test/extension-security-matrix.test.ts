...
const matrix: ReadonlyArray<MatrixEntry> = [
  ...
  {
    relPath: "test/extension-envelope-cancellation.test.ts",
    keywords: [
      "cancellation records and notifies observers",
      "acknowledge removes the cancellation record",
      "negotiation table is frozen and exposes the envelope types",
      "negotiation succeeds when both sides share at least one envelope type",
      "negotiation fails when peerFeatures are malformed",
    ],
  },
  {
    relPath: "test/extension-browser-attach.test.ts",
    keywords: [
      "canonicalOrigin lowercases host and drops path",
      "attach creates a token and never returns cookies",
      "listOpenTabs returns canonical origin and never cookies",
      "setTabLabel rejects unknown tokens and over-long labels",
    ],
  },
  {
